...
def calculate_coordinate(): 
    global global_box_size
    #global_box_size = 40000               # 테스트
    # pi = math.pi
    print ('box_size :',global_box_size)

    distance_factor = 0.0001             # 박스 사이즈 값에 따른 거리 비율 # 수정 필요 .작을 수록 가까이 가야한다.
...
def destination(): 
    ...
    #--------------------Setup parameter-----------------
    #rospy.loginfo('Setting Parameter-> navigation_status = 0') 
    rospy.set_param('mode',0)
    rospy.set_param('navigation_status',0)

    #past_get_param = rospy.get_param('navigation_status')  
    
    rate = rospy.Rate(1) # 발행 속도 10hz 
    while not rospy.is_shutdown():
        global_mode = rospy.get_param('mode_control')


        # 중심에 도착하면 'navigation_status',1 [수정 필요]
        # if abs(global_x_mid - 0.5) < 0.1:
            ...
            if global_navigation_status == 1 : # 순간 오도메트리 , 박스 사이즈 서브 스크라이브
                ...
                while True:
                    ...
                    if global_result == True: # 도착하면
                        rospy.set_param('mode_control',0)
                        global_result = False
